...
trait SupportTicketManager
{
    ...
    {
        $user = $this->user ?? $this->getAuthenticatedUser();

        if (!$user) {
            if (method_exists($this, 'getAuthenticatedUser')) {
                $user = $this->getAuthenticatedUser();
            } else {
                $user = authInfluencer() ?? auth()->user();
            }
        }
        ...
        [
          ...
          'supports' => $supports // collection of SupportTicket
        ]);
        }
        if(authInfluencer()){
    ...
    public function openSupportTicket(Request $r)
    {
        ...
        return Inertia::render('User/Dashboard/Support/AddSupport',
        [
          'pageTitle' => trans($pageTitle),
          'user' => $user , // object of user
          'campain_offer_id' => $campain_offer_id // id
        ]);
    }
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        if(auth()->user()){
            return Inertia::location("/ticket");
        }
        if(authInfluencer()){
    ...
    public function viewTicket($ticket)
    {
        ...
        if(auth()->user()){
        return Inertia::render('User/Dashboard/Support/ViewTicket',
        [
          'pageTitle' => trans($pageTitle),
          'myTicket' => $myTicket , // collection of SupportTicket
          'messages' => $messages, // collection of SupportTicket
          'user' => $user, // collection of SupportTicket
        ]);
        }
        if(authInfluencer()){
    ...
    public function replyTicket(Request $request, $id)
    {
        ...
        return Inertia::location(route('ticket'));
    }
    ...
    public function ticketDownload($ticket_id)
    {
        try {
            ...
            $mimetype = $this->getSafeMimeType($full_path, $ext);
