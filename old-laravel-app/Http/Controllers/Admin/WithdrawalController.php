...
class WithdrawalController extends Controller
{
    ...
    {
        $general = gs();
        $withdrawal = Withdrawal::where('id', $id)->where('status', '!=', 0)->with(['influencer', 'method'])->firstOrFail();
        $pageTitle = $withdrawal->influencer->username . ' Withdraw Requested ' . showAmount($withdrawal->amount) . ' ' . $general->cur_text;
    ...
    public function approve(Request $request)
    {
        ...
        $withdraw = Withdrawal::where('id', $request->id)->where('status', 2)->with('influencer')->firstOrFail();
        $withdraw->status = 1;
