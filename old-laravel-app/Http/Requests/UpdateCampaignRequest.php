...
class UpdateCampaignRequest extends FormRequest
{
    ...
    {
        $campaign = $this->route('id') ? Campaingn::find($this->route('id')) : null;
        return auth()->check() && $campaign && $campaign->user_id === auth()->id();
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'campain_start_date.after_or_equal' => 'La date de début ne peut pas être dans le passé.',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            $campaign = Campaingn::find($this->route('id'));

            // Additional validation for campaigns that already have offers
            if ($campaign && $campaign->campain_offers()->exists()) {
                // Don't allow changing start date if it's too close
                if ($this->campain_start_date !== $campaign->campain_start_date) {
                    $validator->errors()->add('campain_start_date', 'Impossible de modifier la date de début car des offres ont déjà été reçues.');
                }

                // Don't allow reducing budget significantly if offers exist
                if ($this->campain_proposed_budget < ($campaign->campain_proposed_budget * 0.8)) {
                    $validator->errors()->add('campain_proposed_budget', 'Le budget ne peut pas être réduit de plus de 20% car des offres ont été reçues.');
                }
            }
        });
    }
