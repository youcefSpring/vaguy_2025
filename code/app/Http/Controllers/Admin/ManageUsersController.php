...
class ManageUsersController extends Controller
{
    ...
    public function detail($locale, $id)
    {
        ...
        $totalHired = Hiring::where('user_id',$user->id)->count();
        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalTransaction','countries','totalOrder','totalHired'));
    }


    public function kycDetails($locale, $id)
    {
    ...
    public function login($locale, $id){
        ...
        return to_localized_route('user.home');
    }
    ...
    public function status(Request $request, $locale, $id)
    {
        ...
        if ($user->status == 1) {
            ...
            $notify[] = ['success','Client banned successfully'];
        }else{
            $user->status = 1;
            $user->ban_reason = null;
            $notify[] = ['success','Client unbanned successfully'];
        }
        $user->save();
        return back()->withNotify($notify);
