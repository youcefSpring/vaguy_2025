...
// Sidebar dropdown toggle - using event delegation for better reliability
$(document).on("click", ".sidebar-dropdown > a", function (e) {
    e.preventDefault();
    var $this = $(this);
    var $parent = $this.parent();
    var $submenu = $parent.find(".sidebar-submenu").first();

    if ($submenu.length) {
        // Close other open dropdowns
        $(".sidebar-dropdown").not($parent).find(".sidebar-submenu").slideUp(300, function() {
            $(this).removeClass("sidebar-submenu__open");
        });
        $(".sidebar-dropdown").not($parent).find("> a").removeClass("side-menu--open");
