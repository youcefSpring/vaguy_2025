                ...
                <div class="card-body p-0">
                    <div class="table-responsive--md table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tbody>
                            @forelse($agents as $category)
                            <tr>
                                {{-- <td data-label="@lang('S.N.')">{{  $loop->index }}</td> --}}
                                <td data-label="@lang('Name')">{{ __($category->name) }}</td>
                                <td data-label="@lang('Name')">{{ __($category->email) }}</td>
                                <td data-label="@lang('Name')">{{ __($category->username) }}</td>
                                ...
                                <td data-label="@lang('Action')">
                                    ...
                                    <a href="{{localized_route('admin.agent.update',$category->id)}}" class="btn btn-lg btn-outline--warning">
...
<a href="{{localized_route('admin.agent.create')}}" class="btn btn-lg btn-outline--primary">
...
<script>
    (function($) {
            ...
            $('.createButton').on('click', function() {
                modal.find('.modal-title').text(`@lang('Add New Agent')`);
                modal.find('.status').addClass('d-none');
                modal.find('form').attr('action', `{{ localized_route('admin.agent.store','') }}`);
                modal.modal('show');
            });

            $('.editButton').on('click', function() {
                modal.find('form').attr('action', `{{ localized_route('admin.agent.store','') }}/${$(this).data('id')}`);
                modal.find('.modal-title').text(`@lang('Update Agent')`);
