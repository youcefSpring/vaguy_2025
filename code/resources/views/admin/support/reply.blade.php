        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body ">
                    ...
                    <form action="{{ localized_route('admin.ticket.reply', $ticket->id) }}" enctype="multipart/form-data" method="post" class="form-horizontal">
                        @csrf
                            ...
                            <div class="row border border--primary border-radius-3 my-3 mx-2">
                                ...
                                <div class="col-md-3 border-end text-md-end text-start">
                                    ...
                                    @if($ticket->user_id != null)
                                        <p><a href="{{localized_route('admin.users.detail', $ticket->user_id)}}" >&#64;{{ $ticket->name }}</a></p>
                                    @else
                                    ...
                                    data-action="{{ localized_route('admin.ticket.delete',$message->id)}}"
                                    ...
                                    @if($message->attachments->count() > 0)
                                        <div class="my-3">
                                            @foreach($message->attachments as $k=> $image)
                                                <a href="{{localized_route('admin.ticket.download',encrypt($image->id))}}" class="me-2"><i class="fa fa-file"></i> @lang('Attachment') {{++$k}}</a>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ localized_route('admin.ticket.close', $ticket->id) }}">
                        @csrf
...
@push('breadcrumb-plugins')
    <a href="{{ localized_route('admin.ticket') }}" class="btn btn-sm btn-outline--primary"><i class="la la-undo"></i> @lang('Back') </a>
@endpush
