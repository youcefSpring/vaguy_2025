            ...
            @if ($order->status == 4)
                <div class="card b-radius--10 box--shadow1 mb-4 overflow-hidden">
                    ...
                    <div class="card-footer">
                        <p>@lang('Reported by') <a href="{{ localized_route('admin.users.detail', $order->user_id) }}" target="blank">{{ $order->user->username }}</a> (@lang('Client'))</p>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        @if ($order->status == 4)
                            <div class="d-flex justify-content-center gap-2">
                                <button type="button" class="btn btn-outline--danger confirmationBtn" data-action="{{ localized_route('admin.order.action', [$order->id, 6]) }}" data-question="@lang("If you click on the 'yes' button, the client will get the balance. which was deducted during his order and the order will be rejected")" data-btn_class="btn btn--primary">
                                    <i class="la la-user-astronaut"></i>
                                    @lang('In Favour of Client')
                                </button>

                                <button type="button" class="btn btn-outline--success confirmationBtn" data-action="{{ localized_route('admin.order.action', [$order->id, 1]) }}" data-question="@lang("If you click on the 'yes' button, the balance will be added to the influencer account and the order will be completed.")" data-btn_class="btn btn--primary">
                ...
                <div class="card-body p-0">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                            ...
                            <span>
                                <a class="fw-bold" href="{{ localized_route('admin.influencers.detail', $order->influencer_id) }}" target="blank">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                            ...
                            <span>
                                <a class="text--cyan fw-bold" href="{{ localized_route('admin.users.detail', $order->user_id) }}" target="blank">
                                    {{ '@' . $order->user->username }}
                                </a>
                            </span>
                        </li>
        ...
        (function($) {
            ...
            function loadMore(messageCount) {
                $.ajax({
                    ...
                    url: "{{ localized_route('admin.order.conversation.message') }}",
            ...
            $("#messageForm").submit(function(e) {
                ...
                $.ajax({
                    ...
                    url: "{{ localized_route('admin.order.conversation.store', @$order->id) }}",
                    method: "POST",
                    data: formData,
                    async: false,
                    processData: false,
