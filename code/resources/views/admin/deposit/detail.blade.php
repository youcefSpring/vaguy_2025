                ...
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Username')
                            <span class="fw-bold">
                                <a href="{{ localized_route('admin.users.detail', $deposit->user_id) }}">{{ @$deposit->user->username }}</a>
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Method')
                            ...
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    ...
                                    @elseif($val->type == 'file')
                                        @if($val->value)
                                            <a href="{{ localized_route('admin.download.attachment',encrypt(getFilePath('verify').'/'.$val->value)) }}" class="me-3"><i class="fa fa-file"></i>  @lang('Attachment') </a>
                                        @else
                                            @lang('No File')
                                        @endif
                                    @else
                    ...
                    @if($deposit->status == 2)
                        <div class="row mt-4">
                            <div class="col-md-12">
                                ...
                                data-action="{{ localized_route('admin.deposit.approve', $deposit->id) }}"
                                data-question="@lang('Are you sure to approve this transaction?')" data-btn_class="btn btn--primary btn--md"
    ...
    <div id="rejectModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ localized_route('admin.deposit.reject')}}" method="POST">
                    @csrf
