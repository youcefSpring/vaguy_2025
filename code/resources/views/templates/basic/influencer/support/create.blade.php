...
@section('content')
    <div class="card custom--card">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap">
            <h4 class="card-title m-0">@lang('انشاء بطاقة جديدة')</h4>
            <a href="{{ localized_route('influencer.ticket') }}" class="btn btn--outline-custom btn--sm">@lang('بطاقاتي')</a>
        </div>
        <div class="card-body">
            <form class="row gy-3" action="{{ localized_route('influencer.ticket.store') }}" method="post" enctype="multipart/form-data" onsubmit="return submitUserForm();">
                ...
                <div class="form--group col-sm-12">
                    <label for="subject" class="form-label">@lang('الموضوع')</label>
                    <input type="text" id="subject" name="subject" value="{{ old('subject') }}" class="form-control form--control" required>
                </div>
                <div class="form--group col-md-12">
                    <label class="form-label">@lang('الأولوية')</label>
                    <select name="priority" class="form-control form--control form-select" required>
                        <option value="3">@lang('الأعلى')</option>
                        <option value="2">@lang('المتوسط')</option>
                        <option value="1">@lang('المنخفض')</option>
                    </select>
                </div>
                <div class="form--group col-sm-12">
                    <label for="message" class="form-label">@lang('الرسالة')</label>
                    <textarea id="message" name="message" class="form-control form--control" required>{{ old('message') }}</textarea>
                </div>
                <div class="form--group col-sm-12">
                    <label class="form-label">@lang('المرفقات')</label>
                ...
                <div class="col-sm-12">
                    <button class="btn btn--base w-100" type="submit" id="recaptcha">@lang('حفظ')</button>
                </div>
            </form>
        </div>
    </div>
